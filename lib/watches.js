...
bedrock.events.on('bedrock-mongodb.ready', async () => {
  ...
  const indexes = [{
    ...
    collection: COLLECTION_NAME,
    fields: {'watch.mutable': 1, 'meta.watcherLock.expires': 1},
    options: {
      unique: false
    }
  }];
 ...
 */
export async function create({id, watcher, value = null, expires} = {}) {
  ...
  const watch = {
    id, sequence: 0, watcher, mutable: true, value, expires
  };
  ...
  const meta = {
    ...
    // ensure `watcherLock` is always initialized to an expired lock so queries
    // to mark records are simple
    watcherLock: {id: null, expires: new Date(0)}
  };
 ...
 *   minimally set.
 * @param {Date} [options._watcherLockExpires] - Only used in testing.
 * @param {boolean} [options.explain=false] - An optional explain boolean.
 *
...
export async function update({
  watch, _watcherLockExpires, explain = false
} = {}) {
  // build update
  const now = Date.now();
  const expires = _watcherLockExpires ?? new Date(now + ONE_SECOND);
  const update = {};
  update.$set = {
    watch,
    'meta.updated': now,
    // always reset watcher lock when updating; only allow another update after
    // at least one second has passed
    'meta.watcherLock': {id: null, expires}
  };
...
export async function mark({
  ...
  assert.optionalString(id, 'options.id');
  assert.string(watcherLock?.id, 'options.watcherLock.id');
  assert.date(watcherLock?.expires, 'options.watcherLock.expires');
  assert.number(limit, 'options.limit');
  assert.bool(explain, 'options.explain');
  ...
  if(id !== undefined) {
    ...
    // note `meta.watcherLock.expires` is always set on records; it is
    // initialized to use an expiration date in the past
    query['watch.mutable'] = true;
    query['meta.watcherLock.expires'] = {$lt: new Date(now)};
  }
