...
this didn't quite meet the design. Some other p-fun libraries were considered
and it should be possible to substitute some combination of them to implement
the result, but our existing lru-memoize library and the lru-cache library
we use in a number of other places work just fine and were reused here. */
let POLL_CACHE;
...
export async function poll({id, poller, useCache = true} = {}) {
  ...
  const result = useCache ? undefined : POLL_RESULT_CACHE.get(id);
  if(result !== undefined) {
    return result;
  }
