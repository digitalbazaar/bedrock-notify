...
async function _runWatchers() {
  ...
  try {
    ...
    await Promise.all(records.map(async record => {
      ...
      const {value, mutable} = await fn({record});
      if(value !== undefined) {
        await _updateWatchRecord({record, value, mutable});
      }
    }));
  } catch(error) {
...
async function _updateWatchRecord({record, value, mutable = true} = {}) {
  try {
    const sequence = record.watch.sequence + 1;
    const watch = {...record.watch, sequence, value, mutable};
    let _watcherLockExpires;
    if(_TEST_LOCK_EXPIRES_TIME_HOOK) {
      _watcherLockExpires = new Date(_TEST_LOCK_EXPIRES_TIME_HOOK());
    }
    await watches.update({watch, _watcherLockExpires});
  } catch(error) {
